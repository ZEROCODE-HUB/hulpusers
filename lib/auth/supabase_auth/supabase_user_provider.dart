...
class HulpSupabaseUser extends BaseAuthUser {
  HulpSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> hulpSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = HulpSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
